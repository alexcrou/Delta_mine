# ============================================================
#  ЛЕВА МАЙНИНГ v4.0 — ИИ-платформа руководителя проектов
#  (горнодобывающие предприятия: открытые и подземные работы)
# ============================================================
#  СЦЕНАРИЙ РАБОТЫ:
#   ШАГ 1. Пошаговый опрос: лицензия (запрос в Роснедра), способ
#          разработки, инфраструктура, переработка, документы
#   ШАГ 2. Greenfield command center: диагностика проекта,
#          дорожная карта (WBS), матрица документации и экспертиз
#   ШАГ 3. Чек-лист готовности + база знаний на документах
#   ШАГ 4. Ассистент (поиск + опция LLM через OpenRouter)
#   ШАГ 5. PRO: анализ DWG-файлов (платно для пользователей)
# ============================================================
#  ЗАПУСК ЛОКАЛЬНО:   pip install -r requirements.txt
#                     streamlit run app.py
#  ЗАПУСК В ОБЛАКЕ:   Streamlit Cloud (в корне репозитория:
#                     app.py, greenfield_command_center.py,
#                     llm_assistant.py, requirements.txt)
# ============================================================
...
# ---------- ИМПОРТ GREENFIELD COMMAND CENTER ----------
try:
...
# Тема диспетчерской (greenfield-модуль)
if GF_OK:
...
def readiness_check(doc_types_present, project):
    ...
    extra = []
    if not project.get("license_num") and not S.gf_project.get("has_license"):
...
st.caption("ИИ-платформа руководителя проектов горнодобывающего предприятия • v4.0")
...
if menu.startswith("🗺"):
    if not GF_OK:
        st.error("Модуль greenfield_command_center.py не найден в репозитории. "
                 "Загрузите его рядом с app.py.")
    else:
        greenfield_page(S)
...
# ---------- НОВЫЙ ПРОЕКТ: ПОШАГОВЫЙ ОПРОС ----------
    ...
    # ---- ШАГ 5: проект создан ----
        ...
        st.info(f"Документов в базе знаний: {len(S.docs)}. "
                "Переходите в «🗺 Greenfield command center» для дорожной карты, "
                "«✅ Чек-лист готовности» и «🧠 Инженерная логика».")
        ...
        if extra:
            ...
            for dt, status, advice in extra:
                if status == "ok":
                    ...
                    st.markdown(f"&nbsp;&nbsp;&nbsp;→ {advice}")
        st.info("Расширенная нормативная матрица (с обоснованием каждого требования) — "
                "в разделе «🗺 Greenfield command center».")
...
# ---------- ПОДПИСКА ----------
    ...
    st.markdown(f"**Базовая** — {PRICE_BASIC} ₽/мес: опрос, greenfield-центр, документы, чек-лист, база знаний, уставные документы.")
