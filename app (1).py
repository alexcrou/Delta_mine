# ============================================================
#  ЛЕВА МАЙНИНГ v4.1 — ИИ-платформа руководителя проектов
#  (горнодобывающие предприятия: открытые и подземные работы)
# ============================================================
#  НОВОЕ В v4.1:
#   • AI Project Copilot: диалоговый помощник с проектной памятью,
#     наводящими вопросами, подтверждением фактов и действиями
#   • Greenfield command center: дорожная карта, матрица документов
#   • Пошаговый опрос нового проекта
# ============================================================
...
# ---------- ИМПОРТ МОДУЛЕЙ (безопасный: приложение не падает без них) ----------
try:
...
st.caption("ИИ-платформа руководителя проектов горнодобывающего предприятия • v4.1")
...
if menu.startswith("🗺"):
    if GF_OK:
        ...
        st.error("Модуль greenfield_command_center.py не найден в репозитории. "
                 "Загрузите файл в корень репозитория (рядом с app.py) и нажмите Reboot.")

# ---------- AI PROJECT COPILOT ----------
elif menu.startswith("🤖"):
    if COPILOT_OK:
        # Передаём профиль проекта из Greenfield-мастера, если он пройден
        greenfield_profile = S.get("gf_project", {})
        render_ai_copilot(S, S.docs, search_knowledge, greenfield_profile or None)
    else:
        st.error("Модуль ai_project_copilot.py не найден в репозитории. "
                 "Загрузите файл в корень репозитория (рядом с app.py) и нажмите Reboot.")
...
# ---------- НОВЫЙ ПРОЕКТ: ПОШАГОВЫЙ ОПРОС ----------
    ...
    if S.step == 0:
        ...
        st.info(f"Документов в базе знаний: {len(S.docs)}. "
                "Переходите в «✅ Чек-лист готовности», «🧠 Инженерная логика» и «🤖 AI Project Copilot».")
...
# ---------- АССИСТЕНТ (простой) ----------
    ...
    if not S.docs:
        st.info("База знаний пуста. Загрузите документы в разделе «📂 Документы проекта». "
                "Для полноценного диалога с памятью проекта используйте «🤖 AI Project Copilot».")
    else:
        st.caption(f"База знаний: {len(S.docs)} документов.")
        with st.expander("⚙️ Настройки нейросети"):
            if not LLM_OK:
                ...
                idx = models.index(S.model_name) if S.model_name in models else 0
                S.model_name = st.selectbox("Модель (бесплатные)", models, index=idx)
        ...
        if st.button("Спросить") and q.strip():
            ...
            if LLM_OK and S.api_key:
                ...
                if results:
                    st.info("ИИ-ответ требует API-ключ (⚙️ Настройки). "
