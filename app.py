# Изменённый код с поддержкой тёмной/светлой темы
...
# ---------- ТЕМА: CSS-инъекция зависит от S.theme ----------
...
<style>
  ...
  .gd-hero { background: linear-gradient(120deg, #020617, #0c4a6e);
    color: #e2e8f0; border-radius: 18px; padding: 22px 26px; margin-bottom: 16px;
    border: 1px solid #1e3a5f; }
  .gd-hero h1 { color: #f1f5f9; font-size: 1.8rem; margin: 0 0 6px; }
  ...
  .c-ok{background:rgba(22,101,52,.35);color:#86efac;border:1px solid rgba(134,239,172,.4);}
  .c-warn{background:rgba(146,64,14,.35);color:#fcd34d;border:1px solid rgba(252,211,77,.4);}
  .c-risk{background:rgba(153,27,27,.35);color:#fca5a5;border:1px solid rgba(252,165,165,.4);}
  .c-info{background:rgba(7,89,133,.35);color:#7dd3fc;border:1px solid rgba(125,211,252,.4);}
</style>
"""
    return """
<style>
...
S = st.session_state
defaults = {
    "theme": "light",                      # НОВОЕ: тема по умолчанию
...
# Инъекция CSS для выбранной темы (после инициализации состояния)
st.markdown(get_theme_css(S.theme), unsafe_allow_html=True)
...
def requirement_matrix(p):
    ...
    if p.get("capital_construction"):
        ...
        add("Экспертиза ПД (ГГЭ/НГЭ)", "Экспертизы", "Определить маршрут", "По виду объекта и финансированию.")
...
# НОВОЕ: переключатель темы в сайдбаре (после выбора меню)
theme_label = st.sidebar.radio(
    "Тема",
    ["☀️ Светлая", "🌙 Тёмная"],
    index=0 if S.theme == "light" else 1,
    horizontal=True,
)
S.theme = "light" if theme_label == "☀️ Светлая" else "dark"

st.title("⛏ Горный Дельта")
st.caption(f"ИИ-платформа руководителя проектов • v3.0 (единый файл) · "
           f"{'тёмная' if S.theme == 'dark' else 'светлая'} тема")
...
1. **`defaults`**: добавлен ключ `"theme": "light"` — тема сохраняется в session_state между перерисовками.

2. **Сайдбар**: после `menu` добавлен `st.sidebar.radio("Тема", ["☀️ Светлая", "🌙 Тёмная"], horizontal=True)` — выбор записывается в `S.theme`.

3. **CSS**: старый статичный блок заменён на функцию `get_theme_css(theme)`, возвращающую один из двух вариантов:
   - **dark**: фон `#0f172a`, текст `#e2e8f0`, hero-градиент `#020617 → #0c4a6e`, карточки `#1e293b` с бордером `#334155`, чипы — полупрозрачные (`rgba(...,0.35)` фон + светлый текст + тонкий бордер), чтобы `c-ok / c-warn / c-risk / c-info` оставались читаемыми на тёмном фоне;
   - **light**: прежние цвета без изменений.

4. **Порядок исполнения**: CSS инъецируется сразу после инициализации session_state (до рендера контента), поэтому смена темы применяется мгновенно при следующем rerun (radio сам триггерит его). Индикатор активной темы добавлен в `st.caption` под заголовком.
