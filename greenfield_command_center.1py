...
# Интерактивный модуль для Streamlit-приложения «Лева майнинг».
...
# -----------------------------------------------------------------------------
# 1. ДИЗАЙН-СИСТЕМА: «диспетчерская проекта», а не перегруженная админка
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. НОРМАТИВНАЯ/ПРОЕКТНАЯ МАТРИЦА. Условия намеренно прозрачны и редактируемы.
# -----------------------------------------------------------------------------
...
def requirement_matrix(p: dict[str, Any]) -> list[dict[str, str]]:
    """Возвращает объяснимый список документов/согласований.

    Ключи p: mine_method, stage, has_license, reserves_approved, capital_construction,
    infrastructure, processing, tailings, blasting, water_discharge, protected_areas,
    federal_funding, hazardous_facility.
    """
...
# -----------------------------------------------------------------------------
# 3. WBS GREENFIELD-ПРОЕКТА: шаблон задач и зависимости.
# -----------------------------------------------------------------------------

def make_roadmap(p: dict[str, Any]) -> list[dict[str, Any]]:
    start = p.get("target_start") or date.today()
    # id, work, duration, predecessors, phase, owner, critical
    tasks = [
    ...
    if p.get("mine_method") != "подземный":
        # В реальном внедрении заменять шаблон на карьерный/комбинированный, а не просто удалять строки.
        pass
...
def roadmap_health(tasks: list[dict[str, Any]], docs: list[dict[str, str]]) -> dict[str, Any]:
...
# -----------------------------------------------------------------------------
# 4. ИНТЕРАКТИВНЫЙ МАСТЕР: только данные, нужные для первичного маршрута.
# -----------------------------------------------------------------------------

def greenfield_intake() -> dict[str, Any]:
...
# -----------------------------------------------------------------------------
# 5. ЭКРАН РУКОВОДИТЕЛЯ: минимальная когнитивная нагрузка, максимальное действие.
# -----------------------------------------------------------------------------

def render_dashboard(project: dict[str, Any], tasks: list[dict[str, Any]], docs: list[dict[str, str]]) -> None:
...
def render_requirements(docs: list[dict[str, str]]) -> None:
...
def render_roadmap(tasks: list[dict[str, Any]]) -> None:
...
def greenfield_page(state) -> None:
    """state — st.session_state. Добавить страницу «🗺 Greenfield command center»."""
